...
if($ch=="Details")
{
    ...
    if(mysqli_num_rows($recs)!=NULL)
    {
        ...
        while($rec=mysqli_fetch_row($recs))
        {
            ...
            echo"<tr><td>$i</td><td>$row[1]</td><td>$row[3]</td><td>$row[2]</td><td><form method=\"POST\" action=\"T_Past_tours_reg_te.php\"> <input type=\"hidden\" name=\"teid\" value=\"$rec[0]\"><input type=\"hidden\" name=\"tid\" value=\"$t\"><input name=\"ch\"  type=\"submit\" value=\"Details\"></form></td></tr>";
            $i++;
        }
        echo"</table>";
    }
