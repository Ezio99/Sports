...
echo"</style>";
$q="Select te_id from player_team";
...
if(mysqli_num_rows($recs)!=NULL)
{
    ...
    while($rec=mysqli_fetch_row($recs))
    {
        ...
        echo"<tr><td>$i</td><td>$row[1]</td><td>$row[3]</td><td>$row[2]</td><td><form method=\"POST\" action=\"adm_te_det.php\"> <input type=\"hidden\" name=\"det\" value=\"$rec[0]\"> <input name=\"choice\"  type=\"submit\" value=\"Details\"> <input name=\"choice\" type=\"submit\" value=\"Delete\"></form></td></tr>";
        $i++;
    }
    echo"</table>";
}
