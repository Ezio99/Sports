...
if($ch=="Details")
{
    ...
    if(mysqli_num_rows($recs)!=NULL)
    {
        ...
        while($rec=mysqli_fetch_row($recs))
        {
            ...
            echo"<tr><td>$i</td><td>$row[1]</td><td>$row[3]</td><td>$row[2]</td><td><form method=\"POST\" action=\"T_view_reg_team_det.php\"> <input type=\"hidden\" name=\"teid\" value=\"$rec[0]\"><input type=\"hidden\" name=\"tid\" value=\"$t\"> <input name=\"ch\"  type=\"submit\" value=\"Details\"></form></td></tr>";
            $i++;
        }
        echo"</table>";
        }
...
elseif($ch=="Leave")
{
    ...
    if($y!=$cid)
    {
        die("Only the captain can leave a registered tournament");
    }
    $q="Delete from tour_team where te_id='$te' and t_id='$t'";
    mysqli_query($con,$q) or die("error in deleting data".mysqli_error($con));
    echo"Left Tournament";
