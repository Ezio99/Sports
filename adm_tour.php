...
echo"input[type=\"submit\"],input[type=\"reset\"]{background-color: #009999;color: white;padding: 16px 20px;margin: 8px 0;border: none;cursor: pointer;width: 50%;opacity: 0.9;}";
...
if(mysqli_num_rows($recs)!=NULL)
{
    ...
    {
        echo"<tr><td>$i</td><td>$rec[0]</td><td>$rec[4]</td><td>$rec[2]</td><td>$rec[5]</td><td>$rec[3]</td><td>$rec[11]</td><td>$rec[13]</td><td><form method=\"POST\" action=\"adm_tour_det.php\"> <input type=\"hidden\" name=\"det\" value=\"$rec[0]\"> <input name=\"choice\"  type=\"submit\" value=\"Details\">  <input name=\"choice\" type=\"submit\" value=\"Drop\"></form></td></tr>";
        $i++;
    }
    echo"</table>";
}
